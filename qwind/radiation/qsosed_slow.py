...
@njit
def force_multiplier_k_analytical(xi):
    k = 0.03 + 0.385 * np.exp(-1.4 * xi**(0.6))
    return k

@njit
def force_multiplier_eta_max_analytical(xi):
    if(np.log10(xi) < 0.5):
...
@njit
def sobolev_optical_depth_analytical(tau_dr, dv_dr, v_thermal):
    sobolev_length = v_thermal / np.abs(dv_dr)
    sobolev_optical_depth = tau_dr * sobolev_length
    return sobolev_optical_depth

@njit
def force_multiplier_analytical(t, xi):
    TAU_MAX_TOL = 0.001
    k = force_multiplier_k_analytical(xi)
    eta_max = force_multiplier_eta_max_analytical(xi)
    tau_max = t * eta_max
    alpha = 0.6
