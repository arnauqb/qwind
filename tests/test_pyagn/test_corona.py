...
def test_corona():
    ...
    testing.assert_approx_equal(
        sed_test.corona_radius, XSPEC_RHOT, significant=3)
    ...
    testing.assert_approx_equal(sed_test.corona_dissipated_luminosity /
                                sed_test.eddington_luminosity, XSPEC_L_DISS_HOT, significant=2)
    testing.assert_approx_equal(
        sed_test.corona_luminosity / sed_test.eddington_luminosity, XSPEC_LHOT, significant=3)
    testing.assert_approx_equal(
...
def test_corona_luminosity():
    ...
    corona_flux = integrate.trapz(
        x=sed_test.ENERGY_RANGE_KEV, y=corona_spectral_flux / sed_test.ENERGY_RANGE_KEV)
