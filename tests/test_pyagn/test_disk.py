...
def test_eddington_luminosity():
    testing.assert_approx_equal(
        sed_test.eddington_luminosity, 1.257e46, significant=5)
...
def test_gravity_radius():
    testing.assert_approx_equal(
        sed_test.gravity_radius, 1579.9645, significant=3)
...
def test_disk_spectral_radiance():
    ...
    for r in r_range:
        ...
        radiance_2 = np.pi * integrate.trapz(x=sed_test.ENERGY_RANGE_ERG,
                                             y=sed_test.disk_spectral_radiance(sed_test.ENERGY_RANGE_ERG, r))
...
def test_disk_radiance():
    ...
    integral_radiance = const * \
        integrate.quad(lambda r: r * sed_test.disk_radiance(r),
                       sed_test.isco, sed_test.gravity_radius)[0]
    testing.assert_approx_equal(
        integral_radiance, sed_test.bolometric_luminosity, significant=2)
...
def test_disk_truncated_spectral_luminosity():
    ...
    total_lumin = integrate.trapz(
        x=sed_test.ENERGY_RANGE_ERG, y=total_lumin_erg)
    total_lumin2 = integrate.quad(
        sed_test.disk_spectral_luminosity, sed_test.ENERGY_MIN_ERG, sed_test.ENERGY_MAX_ERG)[0]
    testing.assert_approx_equal(total_lumin, total_lumin2, significant=1)
    testing.assert_approx_equal(
        total_lumin2, sed_test.bolometric_luminosity, significant=2)
