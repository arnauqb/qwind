...
def test_initial_parameters():
    ...
    testing.assert_almost_equal(INNER_RADIUS, wind.r_in)
    testing.assert_almost_equal(OUTER_RADIUS, wind.r_out)
...
def test_compute_wind_mass_loss():
    lines = wind.start_lines(v_z_0 = 5e7, niter = 20)
    for line in lines:
        assert line.escaped == False
    lines[5].escaped = True
    mdot_w = wind.compute_wind_mass_loss()
    r = wind.lines_r_range[5]
    dr = wind.lines_r_range[6] - wind.lines_r_range[5]
    mdot_exp = 2 * np.pi * ( (r+dr)**2 - r**2) * wind.Rg**2
    mdot_exp = mdot_exp * lines[5].rho_0 * constants.m_p * lines[5].v_T_0 * constants.c
